...
def create_presentation_outline(context: dict) -> dict:
    ...
    3. When appropriate, include image placeholders using standard markdown image syntax: ![alt_text](path)
    4. For each image placeholder:
    - Use a unique identifier like {{"img_1.png"}}, {{"img_2.png"}} etc. as the path
    - Provide a detailed description of what the image should contain in HTML comments below the image tag
    <!-- Image description: A detailed explanation of what should be generated -->
    5. What you create must be in MARP syntax and should be able to be converted to slide decks using marp cli 
